...
def parse_args(args, parser):
    ...
    parser.add_argument("--other_agent_type", type=str, 
                        default="ppo", choices = ["d3qn","ppo","vi","IDM","mcts"],
    ...
    # rspo parameters
    parser.add_argument('--thre_alpha', type=float, default=0.3,
                        help="The minimal fraction of policy divergence to reject. The smaller the more diverge")
    parser.add_argument('--thre_solid', type=float, default=20)
    parser.add_argument('--smoothing_alpha', type=float, default=0.1,
                        help="updating rate of running acceptance average")
    parser.add_argument('--lambda_factor', type=float, default=1,
                        help="fraction of internal reward")
    parser.add_argument('--resume_from_base',type=str, default='')
    parser.add_argument('--resume_from_iteration', type=int, default=0)
    parser.add_argument('--continuous_from_previous', action='store_true', default=False)
    parser.add_argument('--minus_cross_entropy',action='store_false', default=True)
    parser.add_argument('--mean_for_internal_reward', action='store_false', default=True)
    parser.add_argument('--diversity_step', type=int, default=5,
                        help="Number of diverse policies")
    parser.add_argument('--load_diversity_step', type=int, default=0)
    parser.add_argument('--big_episode', action='store_false', default=True)
...
def main(args):
    ...
    if all_args.share_policy:
        from onpolicy.runner.shared.RSPOhighway_Runner import RSPORunner as Runner
    else:
        raise NotImplementedError
