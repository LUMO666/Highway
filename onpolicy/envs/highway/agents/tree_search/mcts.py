...
from onpolicy.envs.highway.agents.common.factory import safe_deepcopy_env
from onpolicy.envs.highway.agents.tree_search.abstract import Node, AbstractTreeSearchAgent, AbstractPlanner
from onpolicy.envs.highway.agents.tree_search.olop import OLOP
...
class MCTSAgent(AbstractTreeSearchAgent):
    ...
    def make_planner(self):
        ...
        return MCTS(self.env, prior_policy, rollout_policy, self.config,id=self.id)
    ...
    @staticmethod
    def random_available_policy(state, observation, id=None):
        """
        ...
        if id is not None:
            ...
            available_actions=available_actions[id]
            probabilities = np.ones((len(available_actions))) / len(available_actions)

            return available_actions, probabilities, actions
        else:
            probabilities = np.ones((len(available_actions))) / len(available_actions)
...
class MCTS(AbstractPlanner):
    ...
    """
    def __init__(self, env, prior_policy, rollout_policy, config=None,id=None):
        """
            New MCTS instance.
    ...
    def run(self, state, observation):
        ...
        while depth < self.config['horizon'] and node.children and not terminal:
            ...
            if self.id is not None:
                actions, probabilities,self.c=self.prior_policy(state, observation,self.id)
                ass=[]
                for a in self.c:
                    ass.append(self.np_random.choice(a,1)[0])
                action = node.sampling_rule(temperature=self.config['temperature'])
                ass[self.id]=action
                observation, reward, terminal, _ = self.step(state, tuple(ass))
        ...
        if not node.children \
            ...
            if self.id is not None:
                a,b,self.c=self.prior_policy(state, observation,self.id)
                node.expand((a,b))
            else:
